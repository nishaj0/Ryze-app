...
export default function HomeScreen({ navigation }: Props) {
  ...
  const loadData = async () => {
    try {
      const [splitRes, overviewRes, photosRes, suggestionsRes, metricsRes, nutritionRes] = await Promise.all([
        getActiveSplit(),
        getOverview(),
      ...
      // Check if weekly weight log is needed (no entry in the last 7 days)
      const weightMetrics = metricsRes.metrics || [];
      if (weightMetrics.length === 0) {
        setShowWeightPrompt(true);
      } else {
        const latestWeight = weightMetrics.reduce((latest: any, m: any) =>
      ...
      // Check if today's nutrition is logged
      const todayStr = new Date().toISOString().split("T")[0];
      const nutritionLogs = nutritionRes.logs || [];
      const loggedToday = nutritionLogs.some((log: any) => {
        const logDate = new Date(log.date).toISOString().split("T")[0];
        return logDate === todayStr;
      });
      setShowNutritionPrompt(!loggedToday);
  ...
  return (
    <Screen scroll padding="lg">
      ...
      <View style={{ marginBottom: space.lg }}>
        <Typography variant="caption" color={theme.textMuted} weight="600" style={{ letterSpacing: 1, textTransform: "uppercase", marginBottom: space.xs }}>
        ...
        <Card shadow="md" style={{ marginBottom: space.lg }}>
          ...
          {todaySession && todaySession.status === "COMPLETED" ? (
            ...
            <View style={{ flexDirection: "row", gap: space.md, marginTop: space.md }}>
              ...
              >
                <Icon name="Moon" size={20} color={theme.textSecondary} />
                <Typography variant="caption" color={theme.textSecondary} weight="600" style={{ marginTop: space.xs }}>
                  Rest
                </Typography>
              </TouchableOpacity>
            </View>
      ...
      <View style={{ flexDirection: "row", gap: space.md, marginBottom: space.lg }}>
        <Card shadow="sm" style={{ flex: 1, alignItems: "center", padding: space.md }}>
          <View style={{
          ...
          <Typography variant="heading2" color={theme.primary} style={{ fontSize: 28, lineHeight: 1 }}>
            {overview?.currentStreak || 0}
          </Typography>
          <Typography variant="caption" color={theme.textMuted} style={{ marginTop: space.xs }}>
            Day Streak
          </Typography>
        </Card>
        <Card shadow="sm" style={{ flex: 1, alignItems: "center", padding: space.md }}>
          <View style={{
            width: 32, height: 32, borderRadius: radius.full,
            backgroundColor: theme.successBg, alignItems: "center", justifyContent: "center",
            marginBottom: space.sm,
          }}>
            <Icon name="Dumbbell" size={16} color={theme.success} />
          </View>
          <Typography variant="heading2" color={theme.success} style={{ fontSize: 28, lineHeight: 1 }}>
        ...
        <Card shadow="sm" style={{ flex: 1, alignItems: "center", padding: space.md }}>
          <View style={{
            width: 32, height: 32, borderRadius: radius.full,
            backgroundColor: theme.warningBg, alignItems: "center", justifyContent: "center",
            marginBottom: space.sm,
          }}>
            <Icon name="TrendingUp" size={16} color={theme.warning} />
          </View>
          <Typography variant="heading2" color={theme.warning} style={{ fontSize: 28, lineHeight: 1 }}>
      ...
      {showPhotoPrompt && (
        ...
        >
          <View style={{ flex: 1, flexDirection: "row", alignItems: "center", gap: space.sm, marginRight: space.md }}>
      ...
      {showWeightPrompt && (
        ...
        >
          <View style={{ flex: 1, flexDirection: "row", alignItems: "center", gap: space.sm, marginRight: space.md }}>
            <Icon name="Scale" size={22} color={theme.warning} />
            <View style={{ flex: 1 }}>
              <Typography variant="heading3" color={theme.warning} style={{ marginBottom: 4 }}>
                Log Your Weight
              </Typography>
              <Typography variant="bodySmall" color={theme.textSecondary}>
                You haven't logged your weight this week. Stay on top of your progress!
              </Typography>
            </View>
          </View>
          <Button
            title="Log Now"
            onPress={() => (navigation as any).navigate("Profile", { screen: "Metrics" })}
            variant="secondary"
            size="sm"
            fullWidth={false}
          />
      ...
      {showNutritionPrompt && (
        ...
        >
          <View style={{ flex: 1, flexDirection: "row", alignItems: "center", gap: space.sm, marginRight: space.md }}>
            <Icon name="Utensils" size={22} color={theme.success} />
            <View style={{ flex: 1 }}>
              <Typography variant="heading3" color={theme.success} style={{ marginBottom: 4 }}>
                Log Today's Nutrition
              </Typography>
              <Typography variant="bodySmall" color={theme.textSecondary}>
                Track your calories and protein to fuel your training.
              </Typography>
            </View>
          </View>
          <Button
